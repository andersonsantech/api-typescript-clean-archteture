...
export class UserController {
  async createUser(req: Request, res: Response): Promise<void> {
    const userRequest = UserDTO.newInstance(req.body);
    const domain = userRequest.toUser();
    try {
      const user = await createUserUseCase.execute(domain);
  ...
  async updateUser(req: Request, res: Response): Promise<void> {
    const userId: string = req.params.id;
    const userRequest = UserDTO.newInstance(req.body);
    const domain = userRequest.toUser(userId);
    
    try {
      const user = await updateUserUseCase.execute(userId, domain);
      if (user) {
        res.status(200).json(user);
      } else {
        res.status(404).json({ message: "User not found" });
